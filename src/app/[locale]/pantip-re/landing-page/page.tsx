...
export default function Ground() {
  ...
  const FetchLandingPage = useCallback(
    async (
      ...
      try {
        ...
        const response = await fetch(
          'https://pantip.com/api/forum-service/home/get_tag_hit?limit=48',
          {
            headers: {
              accept: 'application/json, text/plain, */*',
              'accept-language': 'en-US,en;q=0.9,ja;q=0.8',
  ...
  useEffect(() => {
    if (selectedTag.slug !== '' || searchString !== '') {
  ...
  return (
    <div
      className={`flex h-screen w-full flex-col px-3 pt-1 md:px-[102px] md:pt-6 ${selectedTag.slug === '' ? 'gap-y-6' : 'gap-y-0'}`}
    >
      <div
